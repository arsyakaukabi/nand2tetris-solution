...
class CodeWriter:
    ...
    def write_arithmetic(self, command: str) -> None:
        if command == "add":
            ...
            true_label = f"BOOL_TRUE{self.label_index}"
            end_label = f"BOOL_END{self.label_index}"
            self.label_index += 1
    ...
    def write_function(self, function_name: str, n_locals: int) -> None:
    ...
    def write_call(self, function_name: str, n_args: int) -> None:
        return_label = f"{self.file_name}$ret.{self.call_index}"
        self.call_index += 1

        self.emit(f"@{return_label}", "D=A")
        self.push_d_to_stack()

        for segment in ("LCL", "ARG", "THIS", "THAT"):
            self.emit(f"@{segment}", "D=M")
            self.push_d_to_stack()

        self.emit("@SP", "D=M", "@5", "D=D-A", f"@{n_args}", "D=D-A", "@ARG", "M=D")
        self.emit("@SP", "D=M", "@LCL", "M=D")
        self.emit(f"@{function_name}", "0;JMP", f"({return_label})")
    ...
    def write_return(self) -> None:
        ...
        for segment in ("THAT", "THIS", "ARG", "LCL"):
            self.emit("@R13", "AM=M-1", "D=M", f"@{segment}", "M=D")
...
def translate_files(vm_files: list[Path], output_path: Path, bootstrap: bool) -> None:
    writer = CodeWriter()
    ...
    for vm_file in vm_files:
        ...
        for op, args in parse_vm_file(vm_file):
            if op in ARITHMETIC_COMMANDS:
                ...
                writer.write_function(args[0], int(args[1]))
            elif op == "call":
                writer.write_call(args[0], int(args[1]))
            elif op == "return":
                writer.write_return()
            else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Project 8 VM Translator (program flow + functions + multifile).",
    )
    ...
    if input_path.is_file() and input_path.suffix == ".vm":
        ...
        bootstrap = len(vm_files) > 1
    else:
        raise SystemExit(f"Unsupported path: {input_path}")

    translate_files(vm_files, output_path, bootstrap)
